...
2. **§Phase 17**, specifically the 17.3 subsection, and **§Phase 17's `17.3 — Done when` list, which is yours. The other three slices have their own and you finish against none of them.**
...
- Finish against **`17.3 — Done when`**, plus the standing rule above it that no endpoint in the module returns a phone number — that one applies to every slice, over the endpoints this one adds.
