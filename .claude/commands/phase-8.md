...
6. **Five questions this phase raises are already answered in §Phase 8 — read the 🔧 notes before designing the schema.** No `Category.priceUnits` column (the enum is the guard; ledger row **P8-1**) · `Category.suggestedTags` **is** yours, seeded from the prototype's twelve-category `TAGS` map · the over-cap publish refusal carries its own code, distinct from the missing-field one, and **no draft-creation cap is built** · `MEDIA_STORAGE=file` copies §0.0 item 17's posture *including* its production refusal · a media row needs reversible moderation visibility, because `photo` is a `Report.targetType`.
