...
### 0.0 Revision 5.35 — read this before §0.1–0.3
...
7. **The twelve categories changed composition in Rounds 25 and 26** (§1c, Phase 4): **Gardening is replaced by Pest Control**, **Computer is renamed Appliance Repair** (broadened to household appliances alongside the computer and device repair it already covered), and 🔧 **Events is replaced by Home Repairs (Round 26)** — small household jobs such as partial painting, tile replacement, mounting and minor carpentry. Home Repairs inherits none of Events' configuration: it quotes on the short window and books on a short lead, and the occasion presets Events carried are gone. Statements below that the category grid exactly matches the original mockup are historical.
...
12. **The island list is real, and island names are not unique — Round 30** (§Phase 7, §Phase 8, §Phase 9). 🔧 **The seed belongs to §Phase 7, not §Phase 4 — corrected 2026-09-09.** This item cited §Phase 4 first and called the list "the seed list §Phase 4 calls for", which §Phase 4 does not: its bullets name no island and its Done-when has no island line. §Phase 7's first bullet is the one that asks for "Island reference data (real seed list, not five entries)", and §Sequencing asks §Phase 4 only for `bookingMode` and `emergencyCapable`. Phase 4 was built to that reading and seeded categories alone, which is correct. The seed list §Phase 7 calls for now exists at `docs/data/inhabited-islands.json`: **192 inhabited islands across 20 atolls**, extracted from the Ministry of Fisheries and Agriculture's atoll register. 🔧 **Fifteen island names occur in more than one atoll and `Meedhoo` exists in three** (Dhaalu, Raa, Seenu), so a name is not an identifier and never was. `Island` carries a UUID like every other entity; nothing — `ProviderServiceArea`, a booking's location, a search filter — may key on the name. **Display convention:** an ambiguous name is qualified with its atoll code in the Maldivian form, `Dh. Meedhoo`, and an unambiguous one stands alone as `Kulhudhuffushi`. The ambiguous set is **computed at seed time** from the case-folded, apostrophe-stripped name — not hardcoded — which yields **16** groups rather than 15 and is the only way `K. Vilingili` and `GA. Vilin'gili` come out distinguishable. **Island search is the primary way an island is chosen, not a filter over a browsable list** — 192 entries is not scrollable. It matches **anywhere in the name** from the first character typed, ranks prefix matches first, matches the atoll code too (`dh mee` → `Dh. Meedhoo`), ignores case, **accents** and the Dhivehi apostrophe on both sides (`male` ≡ `Malé`, `An'golhitheemu` ≡ `Angolhitheemu`), and **returns every match with no cap and no "show more"** — truncating results hides the one island the user is looking for. It must never auto-select, even when exactly one island matches. A native `<select>` is not an acceptable island control anywhere, including `Saved Preferences` and the emergency request form. **No island total appears in UI copy** — the register counts 192 against the 187 the prototypes had quoted, and a denominator that goes stale silently is worth less to a customer than the search itself.
17. 🔧 **SES and real email delivery move to deployment — 2026-09-06.** The app is built first and the email vendor is procured last. `EMAIL_TRANSPORT=file` (built in Phase 2) writes every message as JSON into `backend/.mail/`, so every email-dependent flow — Phase 3's registration and OTP, Phase 3b's reset, Phase 3c's fallback, Phase 10b's alerting — is **built and verified against the file transport**, reading the OTP out of the written message. **This supersedes "SES must be out of its sandbox before this phase starts" (§Phase 3) and "build it, then apply, then start Phase 3" (§4 Sequencing).** Neither an AWS account nor production access is a prerequisite for any phase before deployment. **What does not change:** bounce/complaint handling and the suppression list are already built (item 8 above, delivered in Phase 2); `EMAIL_TRANSPORT=ses` is still refused-unless-set in production; and the production-access request is made **early in the deployment phase, not at the end of it** — AWS answers in about 24 hours but can come back for more information, and it is now the last external dependency in the plan. **The cost, accepted knowingly:** no message reaches a real mailbox until deployment, so a real SES send, a live SNS delivery and the subscription handshake stay unverified until then, and every phase that sends email carries a deferred verification item rather than a closed one. `docs/deferred-verification.md` is the ledger — every phase appends to it and it is worked through at deployment; `docs/decisions/11-email-deferred-to-deployment.md` records this decision.
15. 🔧 **Phase 17's `accept` endpoint does not handle emergency — Round 51** (§Phase 17.1, §Phase 17.3). Item 3 of Phase 17's backend list described `PATCH /v1/bookings/:id/accept` as accepting an emergency "without an amount" and moving it to `accepted` pending the callout fee. That is the two-step, first-acceptance-wins shape **Round 15 removed**, and it sat three lines above the rule that replaced it — `emergency-accept`, which creates an `EmergencyOffer` carrying `calloutFee` and `etaMinutes` together, opens the 90-second collection window, and lets the customer choose from up to three offers. The stale sentence is now rewritten rather than left to §0.0's precedence rule, because a reader who stops at the first of the two never reaches the second.
16. 🔧 **Home Repairs quotes on the short window — Round 51** (§Phase 4, §1d). The per-category quote table listed eight categories and omitted Home Repairs, leaving `quoteExpiryMinutes` / `quoteApprovalMinutes` unset for it. It takes **120 / 240**, with Plumbing, Electrical, AC Repair, Appliance Repair and Pest Control — it is a same-day household trade, it already sits beside those five in the callback-guarantee list (item 10), and nobody waits a day for a quote on a small repair. The long window stays with Photography, Moving and Boat Charter, where planning genuinely happens.
14. 🔧 **The offline queue excludes the emergency accept — Round 46** (§Phase 17, §Phase 17.3). Item 12 below requires the accept prompt to queue and replay on reconnect, and that stands for `PATCH /v1/bookings/:id/accept` — the slot and request prompt it was written for. It does **not** extend to `PATCH /v1/bookings/:id/emergency-accept`, which is a different endpoint doing a different thing: since Round 22 it carries the provider's **callout fee and their own `etaMinutes`** in the same call, and §1f scores on-time rate against that `etaMinutes`. A replayed offer would therefore commit a provider to a price and an arrival promise calculated when they were somewhere else, at some other time, and would be measured against it. The 90-second collection window makes it moot in most cases anyway — a reconnect two minutes later has missed the window even with 28 of the 30 minutes still on the clock. **Offline, the emergency accept control is replaced by an offline notice with a live retry**, so a provider never believes they have bid when they have not. The queue keeps exactly the three surfaces it was built for: the wizard's autosave, the slot/request accept prompt, and chat sends.
13. 🔧 **The slot-mode card affordance is "Pick a time", not "Book instantly" — Round 44** (§1c, Phase 7). Picking a published slot creates a `requested` booking that the provider still has to accept, on the same 24-hour auto-decline clock as a request-based one — so "Book instantly" named an immediacy the state machine does not produce. Nothing about the flow changes: **the label changes, and the screens that had read the label literally are corrected.** The affordance still does the job Round 23 gave it, because the real distinction between the modes survives the rename: in `slot` the customer **picks** from times the provider published, in `request` the customer **asks** and the provider comes back with a time and a price. Statements below that a slot booking is instant, immediate, or confirmed on tapping are superseded; the provider accepting is what confirms it, in every mode.
18. 🔧 **Two things §Phase 9a had to be told twice — 2026-09-15.** Both surfaced when the phase was built, and both were flagged by its builder rather than resolved silently, which is the behaviour this plan wants.

  **The quote-approval clock was still written as a flat 72 hours in five places.** Round 15 split it — `quoteApprovalMinutes`, 240 for the household trades and 4320 for the long-lead ones — and item 8a and §1d have said so since. Five later lines kept the pre-Round-15 figure anyway (§Phase 12's job list, §Phase 9a's reservation bullet, §Phase 17's slice list, its endpoint list and its Done-when), so a phase reading only its own section read 72 hours and had to argue its way back to the rule. **All five now name the field.** The literal 72 hours is not wrong, it is one of the two values: a wedding photographer's quote really does stand for three days, and a blocked drain's stands for four hours. This is the ninth time a derived copy has drifted from a decision and the ninth time the decision was right — the pattern is the point, not this instance of it.

  **A `TimeSlot` is the only row in this system that is removed rather than stamped, and invariant 8 says there are no exceptions.** Both readings cannot stand, so this one is named: a slot is the *expansion of a rule*, not a record of anything that happened, and regenerating a grid after a rule change has to be able to produce a smaller one. The exception is bounded to **future, unreserved** slots and the bound lives in the `deleteMany`'s own `WHERE`, not in its callers — a past slot is somebody's history and a reserved one is somebody's booking, and neither is removable by passing the wrong ids. Everything the slot is derived from — the rule, the exception, the time off — and everything derived from it — the `Reservation` — is soft-deleted exactly as invariant 8 requires. Slot **retention** is knowingly unaddressed: nothing yet prunes a grid the provider stopped using, and the sixty-day horizon bounds how large it gets rather than how long it lives.

19. 🔧 **A subscription payment confirms itself on a full bank-statement match — owner's decision, 2026-09-15** (§1b, §Phase 10a part 2). §1b says RaajjePro collects "via manual bank transfer + admin confirmation", and the transfer stays manual — there is still no gateway. What changes is that **admin confirmation is no longer required for the unambiguous case**. Where an imported bank-statement row matches a `PaymentSubmission` on **all four** of reference code, amount, destination account and billing period, the payment confirms and the entitlement activates without a human, because the statement is the bank's own record and there is nothing left to judge. The measured reason: clearing these by hand costs 5.8 hours a month at 50 providers and **105.0 at 500**, and it is the load that breaks first.

  **Bounded, and the bounds are the whole decision.** The amount is matched against **that provider's** `subscriptionPriceLaari`, never a global figure (§1b's two price points coexist by design). A statement row already matched to another submission cannot match a second — the duplicate check runs before the auto-match, not after. **Anything short of a full match stays manual**, including a near-match, and lands in the existing queue. The action is audit-logged with the **system** as actor, distinguishable from any admin, and every auto-confirmation appears in a reviewable lane the admin can scan and `reverse` exactly as before. 🔧 **The receipt analysis is not an input to this and must never become one** — Round 29 makes it advisory, and automating a decision from an advisory signal is the failure that rule exists to prevent. The statement is the authority; the receipt remains the provider's claim.

20. 🔧 **The admin panel is deferred, and the build moves to §Phase 11 — owner's decision, 2026-09-16.** §Phase 10a **Part 1** (provider billing, Flutter) is built and verified. **Part 2** (the admin panel proper) and **§Phase 10b** are paused: not started, no stack proposed, no `admin/` directory. §Phase 10c is unaffected because it was already sequenced late behind §Phase 21's `ProductEvent` log.

  **What this does not block.** Nothing in §Phases 11–19b depends on it. Suspension's column and its `findVisibleProviders` filter are §Phase 5's and exist; §Phase 10b only adds the admin action. §Phase 17.3 builds the **runtime check** on the contact-reveal kill switch — §Phase 10b builds only the surface that flips it, so 17.3 lands without it. Categories are seeded, and admin editing is a convenience.

  **What it does block, and the sequencing note below is amended rather than left contradicting this.** With no panel, **no payment submission is ever confirmed and no provider ever reaches Bronze**, so nobody becomes paid and nobody becomes verified. Development is unaffected — tests and the database set both directly — but **this cannot ship**. §Phase 10a's own Done-when is four-sixths unmet for the same reason (ledger **P10A-1**), and ledger **P10-DEFER** records the deferral so it is worked through rather than forgotten.

  **The design is separable and is not deferred by this.** `docs/design/sessions/15-admin-panel.md` briefs all eighteen screens with the owner's visual direction and the six 2026-09-15 decisions. Running it blocks nothing and means the build starts from artboards rather than cold.

21. 🔧 **§Phase 17's Done-when is split per slice — 2026-09-16.** It was one paragraph of **thirty-two clauses covering all four slices**, and each of `/phase-17-1` … `/phase-17-4` told its session to "finish against §Phase 17's Done when list". Three-quarters of that list was never any one slice's, so the session building 17.1 had to be told by hand which clauses it owned — a relay that would have been needed again for 17.2, 17.3 and 17.4, and that nobody reading the plan cold would have known to ask for.

  Every clause is preserved and reassigned, none dropped: **17.1** the slot lifecycle, the 24-hour auto-decline, day-7 escalation, Round 24's withdrawal and the offline accept · **17.2** the quote clock at both ends of the 240/4320 split, the provisional hold inside one transaction, and the chat opening at `quote_offered` · **17.3** every emergency clause, including the 30-minute window read from `emergencyAcceptWindowMinutes` · **17.4** recurring, reschedule, Book Again and ICS. The phone rule is stated **once, above all four**, because it binds every slice over the endpoints that slice adds rather than belonging to any of them. The phase-level Done-when is now the sum of the four and is explicitly not a target for any single slice.

22. 🔧 **The request-mode accept window is the category's `quoteExpiryMinutes`, not 24 hours — 2026-09-23.** §1c step 4 said "slot and request-based: 24 hours" while Round 15's table seeded `quoteExpiryMinutes` at **120** for the six household trades and **1440** for the three long-lead ones. Those are the same window and they disagreed, and invariant 13 already settles it: the quote clock is per-category and you **never hardcode 24**. §1c step 4 was the stale half and is corrected in place.

  **The original number is not lost.** The long-lead three are seeded at 1440, which *is* 24 hours, so step 4's figure survives exactly where it was right — Photography, Moving and Boat Charter, where planning genuinely happens. It was only ever wrong for Plumbing, Electrical, AC Repair, Pest Control, Appliance Repair and Home Repairs, where `Request a Time.dc.html` promises the customer two hours by name and the seed agrees with the artboard against the prose.

  **The tell was that nothing read the field.** Before §Phase 17.2, `quoteExpiryMinutes` was seeded with two distinct values and had no consumer anywhere in the system — every reference was CRUD, schema or seed plumbing. A field carrying a real distinction that nothing reads is either dead or the authority, and Round 15 says which. `findAcceptTimeouts` now sweeps `bookingMode: 'slot'` alone; it had listed `['slot','request']` while no request booking could yet exist to be swept. 🔧 **§Phase 17.1's Done-when clause is corrected with it**, having inherited the same stale figure when the Done-when was split on 2026-09-16.
...
9. **The booking chat locks after the job — Round 27**, amending the never-torn-down rule (§1c, Phase 18). The `booking` thread stays the sole channel through the life of the booking **and for 7 days after completion — the callback-guarantee window — then locks read-only**. History is never deleted and stays readable by both parties; a dispute reopens the thread for its duration. Repeat work and later issues route through **Book Again** and the **callback claim**, both of which create real bookings with their own threads. An indefinitely open thread was a free channel for arranging repeat work off the books; statements below that the thread "is never torn down" or "stays open after completion" without limit are superseded.
...
**Applied:** trial starts on the provider's first `confirmed` booking **or** on an explicit provider-initiated "Try Premium" request, whichever comes first. 🔧 **Thirty calendar days — corrected 2026-09-10.** This said 60, which Round 12 halved: §0.0 item 5 and §1b's tier table both read 30, and §1b annotates the change ("Round 12 — was 60"). This section is the historical record of the decision being applied, and the number in it simply went stale. Still one per account.
...
**Payment for jobs is off-platform.** RaajjePro never moves money between customer and provider. Everything RaajjePro collects is a provider paying RaajjePro, via manual bank transfer + admin confirmation — 🔧 **except where an imported bank statement matches the submission on all four of reference, amount, account and period, which confirms without a human (§0.0 item 19); the transfer is still manual and there is still no gateway**. No payment gateway in v1.
...
| `request` | Plumbing, Electrical, AC Repair, Photography, Pest Control, Appliance Repair, Moving, **Home Repairs** 🔧 (Round 26 — replaces Events), **Boat Charter** 🔧 (Round 25 — Pest Control replaces Gardening; Appliance Repair is Computer renamed) | Duration depends on diagnosis, property, negotiation, or trip type — cannot be pre-published as a fixed-length slot |
...
Search results, category results, and Home cards show a mode affordance — 🔧 **"Pick a time"** for slot-based (**Round 44 — was "Book instantly"**), **"Request a time"** for request-based. v3 had no signal at all: a customer expecting to pick a time got a form, or vice versa.
...
🔧 **What the second card signal carries instead — Round 23.** The two booking modes raise different questions, so the signal is mode-appropriate rather than generic:
  - **`slot`** — *how soon can someone come?* → the next open time, e.g. `Next: today 14:00`. 🔧 **Round 44:** it answers *how soon*, which is a real question, and no longer stands as evidence for a promise of instant confirmation — there is no such promise to evidence. A slot booking enters `requested` and waits for the provider exactly as a request-based one does.
...
- 🔧 **Quote and approval windows are per-category — Round 15.** They were a flat 24h to quote and 72h to approve, which meant booking a plumber could legitimately take four days. Nobody waits four days for a blocked drain; they call someone and the booking never happens. **Plumbing, Electrical, AC Repair, Appliance Repair, Pest Control and Home Repairs: 2 hours to quote, 4 hours to approve (🔧 membership updated in Rounds 25–26 — a broken fridge, a bedbug callout and a cracked tile are all closer to a blocked drain than to a wedding).** **Photography, Moving and Boat Charter: 24 hours to quote, 72 hours to approve**, where planning genuinely happens. Seeded as `quoteExpiryMinutes` and `quoteApprovalMinutes` (§1d); never hardcode 24/72.
...
- **Two conversation types, both entirely in-app:**
  ...
  - `booking` — opens automatically at `accepted` and stays open for the entire life of the booking and 🔧 **for 7 days after completion — the callback-guarantee window — then locks read-only (Round 27)**. This is the *only* way arrival logistics, access instructions, and anything not covered in the initial job description gets communicated, and it is never replaced by a "real" contact channel, because there isn't one. After the lock: history stays readable forever, **Book Again** carries repeat work, a callback claim made inside the window already has its own linked booking and thread, and a dispute reopens the thread until it resolves.
...
4. **🔧 Three accept timeouts, not one:**
   ...
   - **Slot:** 24 hours → auto-decline, release the slot, notify the customer to look elsewhere.
   - 🔧 **Request-based: the category's `quoteExpiryMinutes`, not a flat 24 hours — corrected 2026-09-23 (§0.0 item 22).** 120 minutes for the six household trades, 1440 for the three long-lead ones. This line read "slot and request-based: 24 hours" and contradicted invariant 13, which says the quote window is per-category and to **never hardcode 24**. The long-lead figure *is* 1440 — exactly 24 hours — so the original number survives untouched where planning happens; it was only ever wrong for the trades, where `Request a Time.dc.html` promises the customer two hours by name.
   - **Customer quote-approval:** 🔧 **the category's `quoteApprovalMinutes`** from the quote being *offered* (its own clock) — 240 minutes for the household trades, 4320 for the long-lead ones; the flat 72 hours this line used to name is the long-lead figure and was never the rule after Round 15 → quote expires, provisional reservation releases, booking closes. An earlier revision's job auto-declined `quote_offered` 24 hours after *booking creation*, which would have killed quotes the provider submitted at hour 23 before the customer ever saw them.
...
11. Review becomes postable once `completed`. One review per booking. 🔧 **The `booking`-type chat stays open for 7 days after completion — the callback-guarantee window — then locks read-only (Round 27).** History is kept and readable by both parties forever; nothing is deleted. Inside the window a follow-up question or a callback claim has the live thread; after it, the locked thread carries two routes — **Book Again**, so repeat work becomes a booking rather than an arrangement, and the linked callback booking's own thread where a claim was made. A dispute raised after completion reopens the thread for its duration and relocks on resolution. The pre-Round-27 rule that the thread is never torn down is superseded: an indefinitely open thread was a free channel for arranging the next job off the books, and a provider-stated warranty beyond the callback window is honestly served by a Book Again request describing the issue — the provider can quote it at zero.
...
**Cleaning, Plumbing, Electrical, AC Repair, Beauty, Photography, Pest Control, Appliance Repair, Moving, Fitness, Home Repairs, Boat Charter.**
...
- 🔧 **Offered on six categories only — Round 28: Plumbing, Electrical, AC Repair, Appliance Repair, Pest Control and Home Repairs**, read from the category's `callbackEligible` flag (§Phase 4). These are the categories where a fixed or treated problem can recur; on the other six the offer has no referent and the control does not render at all. Note the coincidence worth keeping: this is exactly the short-quote-window group — reactive repair work is quick to quote and the only work a callback can apply to.
- Opt-in per listing **within an eligible category**, displayed as a badge, and a real differentiator against an unrecorded phone arrangement.
...
- Social auth: provider-agnostic interface with stubs. 🔧 **Four providers, not three — Apple was added by the Round 16 mockup audit** (see the Round 16 record: "Apple is correct and the plan is amended, since App Store review requires it wherever third-party sign-in exists"). The set is **Apple, Google, Facebook, Viber**. This bullet listed only Facebook/Google/Viber until 2026-09-08.
...
- **Seed exactly 12:** Cleaning, Plumbing, Electrical, AC Repair, Beauty, Photography, Pest Control, Appliance Repair, Moving, Fitness, **Home Repairs**, **Boat Charter**. 🔧 **Round 25: Pest Control replaces Gardening, and Appliance Repair is Computer renamed and broadened** — the label stays tile-short; the seeded description carries the scope (household appliances, TVs, computers, phones). 🔧 **Round 26: Home Repairs replaces Events** — small household jobs: partial painting, tile replacement, mounting and hanging, sealing and grouting, minor carpentry. Request-based, not emergency-capable; distinct from Appliance Repair, which is devices — Home Repairs is the fabric of the home. 🔧 Boat Charter is new in v5 — picnics, fishing trips, sandbank excursions. Request-based, not emergency-capable; no schema fields beyond the standard set (§1c).
- 🔧 **Also seed, per category:** the `bookingMode` default (§1c); `emergencyCapable` — 🔧 **true for Plumbing, Electrical, AC Repair and Moving** (Moving added in Round 12); 🔧 **`minimumLeadTimeMinutes` — Round 14 set these, do not invent them:** Cleaning 180 · Beauty 120 · Fitness 120 · Plumbing 60 · Electrical 60 · AC Repair 60 · Appliance Repair 120 · Pest Control 180 (🔧 Round 25) · Photography 1440 · Moving 1440 · Boat Charter 1440 · Home Repairs 180 (🔧 Round 26). Only the three slot categories bite immediately, since they are the ones with a picker in Phase 9a; all twelve remain admin-editable from Phase 10b. and `emergencyAcceptWindowMinutes` — 🔧 **30 for Plumbing, Electrical, AC Repair and Moving alike (Round 22 — was 120 for Moving)**, null elsewhere. The 120 described a mover's *arrival*, not their *response*; arrival is now stated per-offer (§1c). 🔧 **`emergencyMinimumTier` (Round 15)** — `gold` for Electrical and Plumbing, `silver` for AC Repair and Moving, null elsewhere. 🔧 **`emergencyEtaPresetsMinutes` (Round 22)** — the arrival options a provider picks from when making an offer (§1c): **[15, 30, 45, 60]** for Plumbing, Electrical and AC Repair; **[60, 90, 120, 180]** for Moving; null elsewhere. Admin-editable from Phase 10b like every other category number, and none of them is preselected in the UI. 🔧 **`quoteExpiryMinutes` / `quoteApprovalMinutes` (Round 15)** — 120 / 240 for Plumbing, Electrical, AC Repair, Appliance Repair, Pest Control and 🔧 **Home Repairs**; 1440 / 4320 for Photography, Moving and Boat Charter (🔧 Rounds 25–26 membership — Home Repairs quotes on 120/240); null for the three slot categories, which do not quote. 🔧 **`callbackEligible` (Round 28)** — `true` for Plumbing, Electrical, AC Repair, Appliance Repair, Pest Control and Home Repairs; `false` for Cleaning, Beauty, Fitness, Photography, Moving and Boat Charter. Gates the wizard's step-6 opt-in, the card badge and the claim path. Admin-editable like every other category field. 🔧 **`occasionPresets` (Round 25)** — the request form's occasion chips (§1c): Photography (Wedding, Birthday, Engagement, Corporate, Graduation, Family, Other) · Boat Charter (Fishing trip, Sandbank trip, Picnic island trip, Sunset cruise, Snorkeling trip, Island hopping, Other); null elsewhere (🔧 Round 26 — Events' presets left with it). Admin-editable like every other category field. Phase 5 reads these, Phases 9/9a/17 consume them.
...
- 🔧 **`priceUnit` is a separate field from the model.** The model says how the price is *calculated*; the unit says what the customer *sees* — `job`, `hour`, `day`, `session`, `visit`. `fixed` + `session` renders "MVR 500/session", which is what the Home and My Services screens already show. Seed a short list per category; never free text, or you get `/session`, `/sesion` and `/per session` on three listings.
  - 🔧 **No per-category column — decided 2026-09-10.** The closed enum is the whole of what this bullet's stated purpose needs; the "short list per category" half has **no subset anywhere in this document**, no keying in the wizard prototype (`Create Service.dc.html`'s `UNITS` is a flat five, rendered whole whatever category step 1 chose, in a file that *does* key its tag suggestions by category), and no consumer in §Phase 10b, whose editable Category fields are enumerated and do not include it. So there is nothing to narrow to, and a `priceUnits` column identical across all twelve rows and editable from nowhere would read to a later consumer as a narrowing feature that does not exist. Contrast `occasionPresets`, which earns its column by being empty outside Photography and Boat Charter. Recorded as an unbuilt half of this bullet in `docs/deferred-verification.md`; §Phase 10b owns the surface if the product ever wants one, and the migration is one additive column with a constant default.
...
- 🔧 **`Category.suggestedTags` — added 2026-09-10, and it is this phase's column.** §Phase 9's step-1 bullet requires "a category-scoped set of suggested tags" as tappable chips, and §Phase 9 is frontend-only — it cannot add a column, and discovering the gap mid-build would stop it. The content is designed, not invented: `Create Service.dc.html`'s `TAGS` map covers all twelve categories, and it is the seed. The listing's own `tags` stay free text underneath the chips, per that same bullet.
...
- Publish endpoint: full required-field validation returning a structured missing-field list. 🔧 **Six required fields, not five:** name, category, short description, at least one island, a pricing model with its price, **and now a cover image.** v4 left the cover image optional at publish, which meant a listing could go live with a blank thumbnail — the first thing a customer sees on every card and in search results. Everything else (gallery beyond the cover, availability detail, extra info) stays optional.
  ...
  - 🔧 **The over-cap refusal must be distinguishable from the missing-field one — 2026-09-10.** §Phase 9 requires an over-cap attempt to show "an upgrade prompt, not a generic error", which the client cannot do if both refusals arrive as the same structured error. Two codes, and the cap one carries the current cap. Note what §Phase 9's line does **not** ask for: a prompt is not a cap, the draft still saves (invariant 2, and "step navigation never blocked"), and the prototype's own over-limit sheet says so — *"This draft is saved and isn't going anywhere"*, fired at publish. No draft-creation cap is built here or anywhere.
...
- Media upload via presigned URL — **server-side content-type and size validation, EXIF stripping on every image**
  - 🔧 **`MEDIA_STORAGE=file` follows §0.0 item 17's posture, guard included — 2026-09-10.** The object store is procured at deployment like the email vendor, so the transport boundary ships with a local backend. Copy the whole posture and not half of it: `EMAIL_TRANSPORT` is **refused unless `ses` in production**, and that refusal is what makes the seam honest rather than decorative. Presigned semantics are simulated by a signed local PUT route.
  - 🔧 **A media row needs reversible moderation visibility, not just soft delete — 2026-09-10.** `photo` is one of §Phase 22's six `Report.targetType` values, so an admin must be able to hide a single image of a listing and put it back (§1d: moderation actions are always reversible, never a hard delete). Invariant 8's soft delete covers the provider removing it; it does not cover moderation.
...
- 🔧 **Trial starts on either trigger, whichever fires first** (§0.4):
  ...
  - 🔧 **The third one prompts; it does not start — decided 2026-09-10.** This clause read "all three call the same `startTrial(providerId)` function", and read that way the 7-day job would spend a provider's trial for them. Three places say otherwise, including the bullet above: §1b's tier table defines trial start as "the first booking reaching `confirmed`, or an explicit 'Try Premium' request, whichever comes first", §0.4 says the same, and the bullet's own verb is **prompt**. The product argument decides it: a trial is one per account and non-renewable, and this job fires *precisely when no booking has landed* — so starting it there spends the provider's only trial when premium is worth least, analytics over no data and priority placement in a market with no demand. It would expire unconverted and depress the trial-to-paid conversion §0.4 calls the single largest lever on subscription revenue. The bullet's own stated problem is **discovery** — "it only fires for a provider who already went looking for it" — and a prompt solves that at no cost.
  - **So two triggers start a trial and one nudges toward the second.** Both starting paths call the same `startTrial(providerId)`, which is a no-op if a trial has ever run for that account; the 7-day job checks the same conditions and sends, and the notice's delivery is §Phase 19's.
...
- **Pause logic shared between `trialing` and `active`** — one function, one 10-cumulative-day cap, resume-remaining-time semantics, forced auto-resume at the cap, applied identically regardless of state.
  - 🔧 **One pause state, two doors — confirmed 2026-09-10.** §1b's "pause keys off the provider-level `acceptingNewCustomers` toggle" is literal: turning the toggle off starts the pause and turning it on resumes, and `POST /v1/providers/me/subscription/pause|resume` reach the *same* function rather than a second one. Two independent pause states would be the same failure as a stored copy of a derived rule, in state form. **At the cap the clock forcibly resumes and the toggle is left alone** — whether a provider takes work is theirs to decide, and only the billing clock is capped. The consequence is accepted and must be **said** rather than discovered: turning off new customers consumes pause allowance and shifts the billing anchor, so the toggle's own copy carries that sentence wherever it renders (§Phase 5's profile surface, §Phase 10's dashboard, §Phase 10a's billing screen).
  - 🔧 **A clock that starts under an already-off toggle starts paused — decided 2026-09-14.** "Keys off" is a rule about a *value*, and reading it only when it changes leaves one case uncovered: the toggle renders today in §Phase 6a's onboarding step 2, where turning it off correctly does nothing because there is no clock to stop. When a trial or a paid period later begins, no transition fires, and the provider is left `trialing`, not accepting customers and **not paused** — burning trial days while taking no work, which is the one thing this pause exists to prevent. So every place a clock starts reconciles against the toggle. **An exhausted allowance does not refuse the start**; the provider simply runs unpaused, because nobody asked for a pause and failing a trial start over it would be absurd.
  - 🔧 **Where the billing sentence belongs, and where it does not — decided 2026-09-14.** It is owed by §Phase 10's dashboard and §Phase 10a's billing screen, where the toggle renders beside a live subscription. **§Phase 6a's onboarding copy stays as it is**, because there it is accurate: that provider has no subscription and no clock, so a billing warning would describe a consequence that does not exist, and §Phase 6a deliberately keeps pricing out of the flow. **The accepted residue:** a provider who turns the toggle off during onboarding and later starts a trial begins it paused without having been told, and their allowance drains from day one. Bounded — ten days, after which the clock forcibly resumes — and in their favour, since the alternative is spending those trial days while taking no work.
- **Scheduled jobs** (Phase 0's runner, not check-on-read): 7-day-out warning, expiry → grace, grace → downgrade, **win-back notifications at 7 and 30 days post-downgrade**, 🔧 **and the introductory-rate conversion — added 2026-09-10, a fifth job this list had omitted.** §1b honours the introductory rate for 12 months from the billing anchor, then converts to standard with 30 days' notice. No other phase owns it: §Phase 10a is the billing UI and §Phase 10c only *measures* the conversion, so leaving it here unnamed meant nobody built the thing being measured. The job decides and re-prices `subscriptionPriceLaari`; the notice's delivery is §Phase 19's, so that half is a seam like the booking hook.
...
- 🔧 **RaajjePro's own bank details are typed configuration — decided 2026-09-10.** §1b step 2 shows them and §Phase 10a renders them, but no entity holds them and §Phase 10b's config list does not include them. They are `BILLING_BANK_NAME` / `_ACCOUNT_NAME` / `_ACCOUNT_NUMBER`: unset in development and **required in production**, the posture `EMAIL_TRANSPORT` and `MEDIA_STORAGE` already take, so nothing fabricates an account number and no payment screen can ship pointing at a placeholder. An admin-editable surface would be building ahead into §Phase 10b.
...
- **Offline resilience:** every step's autosave PATCH is queued locally on failure and replayed on reconnect. **Step navigation is blocked until the current step's data has persisted** — the wizard is the highest-value conversion flow in the app and must not silently lose work on a weak atoll connection.
  - 🔧 **The app-wide no-connection state belongs here — added 2026-09-08.** `App States.dc.html` carries two states: session-expired, which Phase 3 built with its auth state machine, and **no-connection** — the reconnect line, the "Still offline" persistence line, and the explainer naming which actions survive a dropped connection. This phase is its home because this is where queue-and-replay first exists and therefore the first point at which the explainer's claims are true. **Its list grows rather than being written once:** Phase 17.1 adds the slot and request accept prompt, Phase 18 adds message sending, and Phase 17.3's emergency accept is explicitly excluded (§0.0 item 14) — the screen must say so, since an offer needs a live connection. Until this was written down the state had no owning phase at all, having been designed in Session 8 alongside a screen Phase 3 owned.
...
- **Provisional reservations** (§1c) for offered quotes, expiring at 🔧 **the category's `quoteApprovalMinutes`** — read per category and never hardcoded (invariant 13) — swept by a scheduled job. The flat "72-hour expiry" this line used to name predates Round 15, which split the window: 4 hours to approve a plumber's quote, 72 for a wedding photographer's.
...
- 🔧 **Receipt analysis rendered beside the proof image — Round 29.** Each uploaded receipt is checked against the submission and the findings shown as a short per-field list the admin reads before deciding:
  - **Reference code** — present on the receipt and matching the submission's code
  - **Amount** — matching the period's price for *this provider* (`subscriptionPriceLaari`, §1b — never a global constant, so the check reads the provider's own rate)
  - **Destination account** — matching RaajjePro's account
  - **Date** — plausible for the period being paid
  - 🔧 **Duplicate** — this image, or this transaction reference, already submitted against another period. The clearest fraud signal available and the one a human eye misses most easily across a queue.
  - Each finding is `matches` / `does not match — <what was found vs expected>` / **`couldn't read`**. The third is a first-class outcome, not an error: these are phone photos of bank-app screens across several banks, and extraction failing is ordinary. A design that only models match/mismatch will mislead on the commonest case.
- 🔧 **Advisory only, and stated as such in the UI.** The analysis never uses the word "verified", never auto-confirms, never blocks or pre-fills the confirm/reject decision, and carries no aggregate verdict — no score, no traffic light, no "looks good" summary. It is evidence for a person, in the same spirit as §1f: show what was found, let the reviewer conclude. An admin who learns to trust a green summary is how a forged or wrong receipt gets confirmed.
- 🔧 **The bank-statement CSV outranks the receipt.** Where a CSV row matches the reference code, that row is the authoritative record and the receipt analysis is secondary — the statement is the bank's own account of what arrived, the receipt is only the provider's claim about it. Where the two disagree, show both and say which is which.
...
- 🔧 **A full match confirms itself (§0.0 item 19).** Reference code, amount against that provider's `subscriptionPriceLaari`, destination account and billing period all matching one unclaimed statement row → confirmed, entitlement activated, audit-logged with the **system** as actor. A separate **"confirmed automatically"** lane lists them newest first so the admin can scan a week in one pass and `reverse` any of them. A partial match is not a match: it goes to the queue like everything else
- 🔧 **Rejection reasons are a picklist, not a typed sentence.** Wrong amount · unreadable image · reference code missing or wrong · already submitted for this period · account does not match — each with an optional free-text addition. Rejection copy reaches a provider, and a picklist is what stops it varying with how tired the admin is at item thirty
- 🔧 **Every consequential action carries an undo window** — roughly ten seconds, on confirm, reject and reverse. `reverse` already exists as an audit-logged action, so this is an affordance over it rather than new machinery, and it is what makes moving quickly through a queue safe
...
- **Emergency-eligible categories remain code-defined** (§1c: Plumbing, Electrical, AC Repair and 🔧 **Moving — added 2026-09-10; Round 12 made it capable and this line had never caught up**). This list is a safety decision, not configuration, and the owner reaffirmed that on 2026-09-10 rather than opening it to the panel.
  - 🔧 **What that means for the API, made true the same day.** §Phase 4 seeds `emergencyCapable` and `emergencyMinimumTier` as columns because every consumer reads them instead of hardcoding a list — but `PATCH /v1/admin/categories/:id` and `POST /v1/admin/categories` **refuse both fields** with `CATEGORY_FIELD_CODE_DEFINED`, so a thirteenth category cannot admit itself to dispatch either. Refused rather than ignored: an admin told nothing has no way to learn the change did not take. The numbers *around* eligibility stay editable — lead time, the accept window, the ETA presets, both quote windows, `callbackEligible`. Admitting a category decides who is sent to a stranger's home at 2am on a gas leak; the rest is scheduling.
...
- 🔧 **Saved views** — a named filter, sort and date range pinned to the sidebar ("overdue payments", "identity, oldest first"). Per-admin, not shared; it removes the re-filtering tax on every visit and nothing depends on it
...
- 🔧 **Who is due, and who is overdue.** Billing anchors are per-provider and **pause shifts the anchor by the paused duration**, so after a few pauses no two providers share one and nobody can work out by eye who owes what. A due-this-week and overdue list read off the anchor, each row carrying that provider's own `subscriptionPriceLaari` — the introductory and standard rates sit side by side and must not be summed into an average
- 🔧 **In-flight emergency monitor.** Open emergency requests as they run: broadcast time, eligible providers reached, offers collected so far, the 90-second collection window, and the 30-minute response deadline counting down. §1c's dispatch **never targets a provider**, so without this nobody is watching a request die quietly — and an emergency that reaches nobody is the worst failure this product has. Read-only: there is no admin action on a live emergency, and adding one would be the force-override §Phase 10b declines
...
- 🔧 **Booking-mode affordance on every result card** (§1c): 🔧 **"Pick a time" (Round 44 — was "Book instantly")** / "Request a time", plus the mode-appropriate second signal and the callback-guarantee badge (Round 23) 🔧 **where the category is `callbackEligible` — Round 28**. 🔧 **No "Emergency available" marker and no emergency filter** — Round 23 removed both; a filter that narrows by something the customer cannot act on produced dead-end result sets
...
- **17.2 — request-based and quotes:** quote offer/approve, provisional reservations, the quote-approval job 🔧 **on the category's `quoteApprovalMinutes` clock, not a flat 72 hours**
...
- **17.4 — recurring series and reschedule**, and 🔧 **Saved Preferences — reattributed here 2026-09-10, by the owner's decision.** §Phase 3 collected it as an account-settings bullet and deferred it because labelled addresses need `Island`; §Phase 7 then seeded `Island` and declined it, correctly, because §Phase 7's own bullets and Done-when name it nowhere and no section gave it an entity shape or endpoints. §1h is what asks for it — saved addresses, preferred time windows and standing instructions, "reused by every booking flow" and "carried forward by **Book Again**" — and Book Again is this slice. Before bookings exist a saved preference has nothing to be used by. The design is already drawn: `mockups/design-composer/Saved Preferences.dc.html`.
...
3. `PATCH /v1/bookings/:id/accept` — sets `agreedAmount` for slot and request bookings. 🔧 **Emergency does not use this endpoint — Round 15.** It has its own, `emergency-accept` in item 4 below, which creates an `EmergencyOffer` carrying the provider's `calloutFee` and `etaMinutes` together, opens the 90-second collection window, and leaves the customer to choose from up to three offers. *Superseded text: this item previously read "for emergency, accepts without an amount and moves to `accepted` pending the callout fee" — that is the two-step, first-acceptance-wins shape Round 15 removed, and it sat three lines above the rule that replaced it.* No contact info is ever exposed by this or any later step (§1c) — the `booking`-type chat opens here instead (Phase 18).
...
7. **Scheduled job — accept timeouts, three distinct conditions:** emergency `requested` older than **30 minutes**; slot/request `requested` older than **24 hours**; `quote_offered` older than 🔧 **the category's `quoteApprovalMinutes` from the quote timestamp** (not from booking creation, and not a flat 72 hours — Round 15). Each releases its reservation and notifies the correct party.
...
🔧 **Done when — by slice, split 2026-09-16.** This was one paragraph of thirty-two clauses covering all four slices, and `/phase-17-1` through `/phase-17-4` each told their session to "finish against §Phase 17's **Done when** list". Three-quarters of it was never theirs, and the session building 17.1 had to be told by hand which clauses it owned. **Phase 17 is done when all four slices are.** Each slice finishes against its own list below and against no other.

**Standing across all four slices, not a clause of any one of them:** 🔧 **no endpoint in the module returns a phone number, WhatsApp handle, or Viber handle — with the single exception of `reveal-contact`, and no WhatsApp or Viber handle even there** — verify this by inspecting every response shape in the module, not just the ones expected to carry it. Every slice re-checks this over the endpoints it adds.

**17.1 — Done when:** concurrent bookings on one reservation window resolve to one winner **even across different listings of the same provider**; an unresponsive provider auto-declines at **24 hours** for slot (request is §Phase 17.2's, on the category's `quoteExpiryMinutes` — §0.0 item 22); an unresolved payment claim escalates to admin review at day 7 without unlocking anything; 🔧 **a payment claim can be withdrawn while unanswered and the booking returns to `awaiting_payment`; the withdrawal is rejected once the provider has confirmed or disputed, and rejected on a second attempt; both transitions remain visible in `statusHistory`; a withdrawal files no Report and moves no conduct metric (Round 24)**; the completion prompt distinguishes confirmed from unconfirmed completions; an accept tapped in airplane mode replays on reconnect rather than being lost.

**17.2 — Done when:** a request-mode listing takes a booking at all — the nine `request` categories are refused by 17.1 by name and this is what opens them; 🔧 a quote expires on its own clock — the category's `quoteApprovalMinutes`, asserted at **both** ends of the split so a 4-hour window and a 72-hour one are each seen to expire on time — and releases its provisional reservation; the provisional reservation is created **inside** the quote transaction and released in one, the property 17.1 proved for the firm hold; the `booking`-type chat **opens at `quote_offered`**, not at `accepted` (§0.0 item 7) — 17.2 owns the state, §Phase 18 owns the thread.

**17.3 — Done when:** the reveal endpoint rejects a non-emergency booking, a `requested`-state booking, and a provider-initiated call, reveals both numbers or neither, and returns nothing 24 hours after the booking goes terminal; revoking a provider's verification auto-cancels their `accepted` emergency bookings but routes their `payment_claimed` ones to admin instead; an emergency booking on an ineligible category or by an unverified provider is rejected; the emergency rate limit triggers; 🔧 a broadcast reaches every eligible provider and nobody outside the eligibility rule; 🔧 two simultaneous accepts both produce offers rather than one winner, and the customer is shown both; a collection window closes at 90 seconds and presents at most three offers; selecting one releases the others immediately; reject-all re-broadcasts and never returns to any provider who offered; an unanswered set of offers expires 5 minutes after the window closes and re-broadcasts without resetting the overall window; 🔧 selecting an offer incurs the MVR 200 dispatch fee, the job proceeds without waiting for payment, an unsettled fee blocks a new booking, and submitting proof lifts that block before any admin confirms it; 🔧 a provider marked as not arrived is released, takes a no-show against conduct, and the booking re-broadcasts without a second fee; 🔧 an emergency on Electrical is refused to a silver provider and accepted from a gold one, while AC Repair accepts silver; 🔧 the request window expires at **30 minutes for Moving exactly as for Plumbing** (Round 22), and each offer carries the provider's arrival estimate; an emergency job cannot be completed without a final amount, and the endpoint rejects the attempt; the "did this happen" flow fires for emergency bookings too; the emergency `requested` window auto-declines at **30 minutes**, read from `emergencyAcceptWindowMinutes`.

**17.4 — Done when:** a missed recurring occurrence skips rather than kills the series; reschedule manages reservations atomically; "Book again" opens a correctly-routed new request against the same listing; a confirmed booking exports a valid ICS entry.

**The phase — done when:** all four of the above hold, and the full lifecycle works for all three modes; the `booking`-type chat stays open through and after completion (§Phase 18 builds the thread).

### Phase 18 — Messaging Module

- `Conversation` type **`enquiry`** (listing-scoped, available before any booking) or **`booking`** (opens at `accepted`, 🔧 **stays open for the life of the booking and for 7 days after completion, then locks read-only — Round 27**; it is the only coordination channel that ever exists between these two parties while the job lives, not a waiting room before a "real" contact exchange, because there is no contact exchange, §1c). 🔧 **Locked-thread rendering (Round 27):** full history read-only, a plain closed-job notice, and two routes out — **Book Again**, and the linked callback booking's thread where a claim exists. An open dispute reopens the thread until resolved; `payment_unresolved` is non-terminal and never locks.
...
- 🔧 **The Email notifications block on Account Settings — added 2026-09-08.** The `Marketing` and `Weekly digest` toggles, under the fixed line **"Booking updates are always sent — they keep your bookings running and have no switch"** (§Phase 3c: transactional sends have no in-app toggle). Designed in Session 9 as part of `Account Settings.dc.html` and deliberately left unbuilt by Phase 3, whose account-settings list does not include it. **This phase's own Done-when requires the digest to send "to opt-in providers only", which is not possible without the control that records the opt-in** — and until this was written down, no phase's frontend scope contained it. Found auditing the built Phase 3 screens against the prototypes.
...
- **Phase 10b depends on Phase 10a only** (shares its app shell, auth, and audit log) and ships right after it — it carries the operational surface needed from first launch. 🔧 **Suspended 2026-09-16 by §0.0 item 20**: Part 2 and 10b are deferred and the build moved to §Phase 11. "Ships right after it" is the order to return to, not a description of what is happening — and "needed from first launch" is unchanged and is what makes this a deferral rather than a cut.
...
*Two business values set.* The **introductory rate is MVR 75**, half the standard MVR 150, held for the first 100 providers for 12 months — the spread chosen to be wide enough that Phase 10c's cohort comparison actually reads. **Minimum lead times** are seeded per category from 60 minutes for the three trades to 1440 for the planning categories (🔧 Round 26 — the 2880 ceiling left with Events); only the three slot categories bite at launch, since they are the ones with a picker.
