...
### 0.0 Revision 5.9 — read this before §0.1–0.3
...
| **On-time rate** | arrivals within 15 min of `scheduledFor` ÷ completed with an arrival mark | Slot and request modes. 🔧 **Round 22 removed the reason emergency was excluded** — an emergency offer now carries the provider's own `etaMinutes`, so a promised arrival exists to measure against. Whether to fold emergency into this metric is an open decision, not a settled exclusion; do not build it either way until it is taken |
