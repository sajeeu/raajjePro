...
7. Minimal CI (GitHub Actions is fine) running lint + build on both apps on push, PLUS automated dependency scanning (Round 15) — it is free here and covers the most common real compromise path. No deployment yet.
7b. POINT-IN-TIME RECOVERY configured from the first migration (Round 15): WAL archiving with RPO of 5 minutes or better. Booking payment is a self-attestation RaajjePro never independently observes, so the database is the ONLY record that money changed hands — 24 hours of loss is 24 hours of unadjudicable disputes. Turning PITR on later cannot recover what was lost before it was on.
8. .env.example covering DB connection, JWT secrets, object storage (TWO buckets — media, and a separate private bucket for identity documents), AMAZON SES credentials and region (the transactional email provider — load-bearing) and push (FCM/APNs) placeholders — NO SMS provider, deliberately. No real secrets committed.
9. EMAIL BOUNCE AND COMPLAINT HANDLING, built here and not at Phase 3c. An SNS event destination receiving SES delivery/bounce/complaint notifications, a stored per-message result, and a suppression list consulted BEFORE every send. This is in Phase 0 for a hard external reason: SES will not grant production access until the account attests that this handling already exists, and a sandboxed account sends only 200 messages/day to pre-verified addresses — so Phase 3's registration flow cannot even be TESTED until it clears. Request SES production access as soon as this is working; AWS answers within 24 hours but may ask for more information.
...
BUILD RTL-READY WITHOUT SHIPPING DHIVEHI (Round 15). Thaana is right-to-left, and localisation is deferred — but RTL in Flutter is Directionality, EdgeInsetsDirectional, start/end and mirrored motion, NOT a translation file. A design system built with absolute directions is inherited by every later phase and the retrofit becomes a presentation-layer rewrite. Use directional insets and alignment EXCLUSIVELY: no EdgeInsets.only(left:), no TextAlign.left, no hardcoded row order. This costs nothing now.

Definition of done: the component gallery route renders every widget in every state; the gallery also renders correctly under a forced RTL Directionality with no overlap or clipping; toggling the OS reduced-motion setting visibly disables shimmer and transition animation; a screen reader announces each interactive widget meaningfully.
```
...
3. EMERGENCY PROMPTS SEND EMAIL UNCONDITIONALLY, in parallel with push, not just on push failure — a 30-minute response window cannot absorb a silent push failure.
   EMAIL IS NOW THE ONLY CHANNEL THAT WORKS WHEN PUSH FAILS, so its deliverability is load-bearing. BOUNCE AND COMPLAINT HANDLING ALREADY EXISTS — it was built in Phase 0, because SES will not leave its sandbox without it. Do not rebuild it here. What this phase adds is the consumption side: honour the suppression list before sending, and watch the reputation metrics rather than merely collecting them. AWS places an SES account under review above a 5% bounce rate and can pause sending above 10%, so this is the channel's own survival, not just a metric.
...
6. EMAIL is three separately controllable channels, not one: OTP email, notification/fallback email, and marketing email. Back them with THREE SES CONFIGURATION SETS, which is also what keeps their reputation metrics separate — a complaint spike on the opt-in digest must not degrade the sending reputation OTP depends on. Phase 10b attaches kill switches to each independently. Do NOT build a single global email toggle — it would take down authentication, since email is now the ONLY OTP channel and the only fallback.
...
1. Category entity: id, name, slug, icon reference, sortOrder, active flag, and THREE fields later phases read as defaults:
   ...
   - emergencyMinimumTier: enum or null (Round 15) — the MINIMUM verificationTier for emergency work in this category
   - quoteExpiryMinutes / quoteApprovalMinutes: integer or null (Round 15) — request-mode quote and approval windows
2. SEED EXACTLY 12 CATEGORIES, in this order, with these values. The mockup's grid is 3 columns × 4 rows and the count matters:
   ...
   SEED emergencyMinimumTier (Round 15): GOLD for Electrical and Plumbing — electrical and plumbing failures at 2am are life-safety work in a stranger's home, and Gold is the only tier carrying a trade certificate. SILVER for AC Repair and Moving. NULL elsewhere. NEVER hardcode 'silver' as the emergency bar.
   SEED quoteExpiryMinutes / quoteApprovalMinutes (Round 15): 120 / 240 for Plumbing, Electrical, AC Repair and Computer — a flat 24h/72h meant booking a plumber could take four days, and nobody waits four days. 1440 / 4320 for Photography, Gardening, Moving, Events and Boat Charter, where planning genuinely happens. NULL for the three slot categories, which do not quote.
...
3. THE REQUEST BROADCASTS TO EVERY ELIGIBLE PROVIDER AT ONCE — emergency-capable category, island match, acceptingNewCustomers on, and verificationTier MEETING THE CATEGORY'S emergencyMinimumTier (Round 15: GOLD for Electrical and Plumbing, SILVER for AC Repair and Moving — read the field, never hardcode 'silver'). Older documentation sent an emergency to ONE provider and deferred fan-out to post-v1; that is reversed. Sequential dispatch is the wrong shape for the one booking type where minutes matter.
...
4b. ACCEPTANCES DO NOT RACE — OFFERS COLLECT, THEN THE CUSTOMER PICKS (Round 15 replaced first-past-the-post). The FIRST acceptance opens a 90-SECOND COLLECTION WINDOW rather than winning the booking. Every other eligible provider may accept with their own fee during it, each creating an EmergencyOffer row (bookingId, providerId, calloutFeeLaari, createdAt, state). At the end of the window the customer is shown UP TO THREE OFFERS SIDE BY SIDE — provider, tier, rating, distance, callout fee — and selects one.
   Under the old rule the winner was whoever tapped fastest rather than whoever was nearest or cheapest, the customer decided blind one offer at a time under a countdown, and the dominant provider strategy was to accept instantly and quote high. 90 seconds out of a 30-minute budget converts a race into a competitive bid.
   - emergency_offered NOW MEANS "offers are collected and awaiting the customer", NOT "one provider has claimed this". Older documentation saying otherwise is obsolete.
   - Fewer than three offers at window close → show what arrived. NONE → keep broadcasting, open a new window on the next acceptance.
   - PATCH /v1/bookings/:id/emergency-offer-response takes the chosen offerId, or reject-all.
   - Select → agreedAmount = that offer's calloutFee, amountKind 'callout_fee', status → awaiting_payment. Unselected providers are released and told immediately, never left on a spinner.
   - Reject-all → back to requested, RE-BROADCAST, and add every offering provider to rejectedProviderIds so they are excluded from later rounds.
   - SCHEDULED JOB: a collection window whose customer does not respond within 5 MINUTES of close releases all offers, returns to requested and re-broadcasts.
4c. MVR 200 EMERGENCY DISPATCH FEE (Round 15) — the first and only charge to a customer in this system. Incurred when the customer SELECTS an offer, never on submitting a request: a dispatch nobody answers must cost nothing. It NEVER blocks dispatch — there is no payment gateway, so it is recorded as owed and the job proceeds. Settled afterwards through PaymentSubmission with a new purpose value emergency_dispatch_fee, against a generated reference code.
   - An unsettled fee blocks NEW bookings of any type. Existing bookings are unaffected and the account is not suspended.
   - THE BLOCK LIFTS ON PROOF SUBMISSION, NOT ON ADMIN CONFIRMATION. Online banking here is done from a phone, so settling takes minutes, while admin confirmation runs a 48-hour SLA — never hold a customer against that queue. The admin verifies afterwards; a fabricated receipt is a moderation matter, not a reason to make everyone wait.
   - A re-dispatch after a provider no-show does NOT incur a second fee. One emergency, one fee.
4d. PROVIDER NO-SHOW AND RE-DISPATCH (Round 15). A confirmed emergency provider who never arrives is currently unhandled and the customer simply waits. Allow "provider has not arrived" from the booking screen once the category's accept window has elapsed: release the provider, record a no-show against their conduct record, and RE-BROADCAST immediately excluding them. No admin involvement — an emergency cannot wait for a queue.
