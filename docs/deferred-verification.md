...
| P7-1 | **That the Flutter island controls render the live endpoint's response on a device.** The search rules are verified against the real server over HTTP — `male` and `Malé` both reaching `Male'`, `Angolhitheemu` reaching `An'golhitheemu`, `gdh` listing that atoll, `dhoo` returning 105 matches uncapped, `vilingili` returning both, an empty term returning the whole register — and the response's field names were read off the live JSON and match `Island.fromJson` one for one. What has not been seen is the widget drawing that response on a phone: `island_search_list_test.dart` and `island_multi_select_test.dart` drive it against a fake whose shape was copied from the live response, which is the same class of stand-in that once let Explore render an empty state against a perfectly good 200. | Run the app against a live API (`flutter run -d emulator-5554 --dart-define=API_BASE_URL=http://10.0.2.2:3000`), open the component gallery's **Island picker** section, type `mee`, and see three qualified Meedhoos with their atolls — then the header sheet on Explore, picking one and finding it still on the pill after a trip to Profile and back. Attempted during the build; the emulator and the API do not survive a turn in this session, so it was recorded rather than claimed. |
