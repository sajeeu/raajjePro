...
**`request.principal`.** The admin-session plugin resolves the cookie on every request under `/v1/admin`. A live, unrevoked, unexpired session sets `request.principal = { kind: 'admin', id, sessionId, mfaVerified, totpEnrolled, reauthenticatedAt }` — even before MFA is verified, so rate limiting can key on the admin and the MFA routes can find their session. The guards, not the plugin, decide what that principal may do.
