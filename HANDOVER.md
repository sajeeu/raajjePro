...
**Next**: `/phase-8a` — Subscription & Trial, or `/phase-9` — the
Create/Edit Service Wizard. Phase 8a replaces the body of `activeListingCap`
and its callers do not change. Phase 9 wires the wizard to
`/v1/providers/me/listings`, reads its chips from `Category.suggestedTags`
rather than building a second copy of the map, and renders the server's own
`emergency.reason` on step 5 rather than recomputing the rule.

| | |
|---|---|
| `01_Development_Plan_v5.md` | **The single source of truth**, revision 5.25. Every product decision. Read §0.0 first — it is a precedence rule |
...
- **`verify-dc.py` is red on purpose, and stays red until one design round lands (2026-09-10).** `Create Service.dc.html` step 5 still tells a provider a slot booking is "confirmed instantly", which Round 44 retired — §0.0 item 13: picking a slot creates a `requested` booking the provider must accept. The gate's Round 44 pattern was written against the three phrasings visible at the time and missed this fourth one for a dozen rounds; it is widened now, so the failure is the rule working rather than a broken script. **Fix it by importing `docs/design/sessions/round-56-slots-are-not-instant.md` into the design project, not by editing the repo copy** — the project is the source, and an edit here would be reverted by the next import. Until then `scripts/verify.sh` and the 16:30 backstop both fail at Prototypes, with the other 60 artboards ok
